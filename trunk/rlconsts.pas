...
{
  LS_PrintingInProgressStr:= UTF8Encode('Imprimindo o relatório...');
  LS_FilterInProgressStr  := UTF8Encode('Salvando o relatório...');
  LS_PreparingReportStr   := UTF8Encode('Preparando o relatório...');
  LS_PrinterNotFoundStr   := 'Nenhuma impressora encontrada';
  LS_NoPathToPrinterStr   := UTF8Encode('Caminho inválido para a impressora');
  LS_LoadDefaultConfigStr := UTF8Encode('Será carregada a configuração padrão');
  LS_PrinterDriverErrorStr:= 'Erro no driver da impressora';
  LS_PageStr              := UTF8Encode('Página');
  LS_PrepareErrorStr      := UTF8Encode('Erro durante a preparação do relatório');
  LS_PageBreakStr         :='Continua...';
  LS_PageMendStr          := UTF8Encode('Continuação');
  LS_ReportEndStr         :='Fim';
  LS_FileNotFoundStr      :='Arquivo não encontrado';
  LS_FileNameStr          :='Nome do arquivo';
  LS_AllFileTypesStr      :='Todos os arquivos';
  LS_LoadReportStr        := UTF8Encode('Carregar relatório');
  ...
  LS_PrintStr             :='Imprimir';
  LS_AboutTheStr          :='Sobre o';
  LS_PreviewStr           := UTF8Encode('Pré-visualização');
  LS_OfStr                :='de';
  LS_ZoomStr              :='Zoom';
  LS_FirstPageStr         := UTF8Encode('Primeira página');
  LS_PriorPageStr         := UTF8Encode('Página anterior');
  LS_NextPageStr          := UTF8Encode('Próxima página');
  LS_LastPageStr          := UTF8Encode('Última página');
  LS_EntirePageStr        := UTF8Encode('Página inteira');
  LS_EntireWidthStr       := UTF8Encode('Largura da página');
  LS_MultiplePagesStr     := UTF8Encode('Várias páginas');
  LS_ConfigPrinterStr     :='Configurar impressora';
  LS_SaveToFileStr        :='Salvar em disco';
  LS_SendToStr            :='Enviar para';
  LS_PrinterStr           :='Impressora';
  ...
  LS_RangeToStr           :='até';
  LS_AllStr               :='Tudo';
  LS_PagesStr             := UTF8Encode('Páginas');
  LS_SelectionStr         := UTF8Encode('Seleção');
  LS_CopiesStr            := UTF8Encode('Cópias');
  LS_NumberOfCopiesStr    := UTF8Encode('Número de cópias');
  LS_OkStr                :='Ok';
  LS_DivideScreenStr      :='Dividir a tela';
  LS_InvalidNameStr       := UTF8Encode('Nome inválido');
  LS_DuplicateNameStr     := UTF8Encode('Nome já utilizado');
  LS_UseFilterStr         :='Usar Filtro';
  ...
  LS_XLSFormatStr         :='Planilha Excel';
  LS_AtStr                :='em';
  LS_FormStr              := UTF8Encode('Formulário');
  LS_DefaultStr           := UTF8Encode('Padrão');
  LS_ColsStr              :='cols';
  LS_ZoomInStr            :='Aumentar o zoom';
  LS_ZoomOutStr           :='Diminuir o zoom';
  LS_CopyStr              :='Copiar';
  ...
  LS_FindNextStr          := UTF8Encode('&Próxima');
  LS_WholeWordsStr        :='Palavras &inteiras';
  LS_MatchCaseStr         := UTF8Encode('Diferenciar &maiúsculas de minúsculas');
  LS_DirectionUpStr       :='A&cima';
  LS_DirectionDownStr     :='A&baixo';
  ...
  Ls_Salvar_Como          :='Salvar Como';
  Ls_Nome_Arquivo         :='Nome do Arquivo:';
  LS_OptionsStr           := UTF8Encode('Opções');
  LS_ColumnsStr           :='Colunas';
  Ls_Propriedades         :='Propriedades';
  Ls_Progresso            :='Progresso';
  Ls_Aplicar              :='Aplicar';
  Ls_File_corrupted       :='Arquivo Corrompido';
  Ls_File_version         := UTF8Encode('Versão de arquivo inválido!');
  Ls_Page_setings         := UTF8Encode('Configuração do Papel');
  Ls_Page_margins         :='Margem';
  Ls_Page_margins_top     :='Superior';
  Ls_Page_margins_bottom  :='Inferior';
  Ls_Page_margins_paper   :='Margsss';
  ...
  Ls_Page_margins_rigth   :='Direita';
  Ls_Page_margins_left    :='Esquerda';
  Ls_Paper_Orientation    := UTF8Encode('Orientação');
  Ls_Paper_Orientation_Portrait:= 'Retrato';
  Ls_Paper_Orientation_Landscape:='Paisagem';
  }
end;
